...
static char LFTABLEHEADERViewKey;
static char ShowLFHeader;
...
-(void)setLf_header:(LFTableHeaderView *)lf_header{
    [self willChangeValueForKey:@"LFTABLEHEADERViewKey"];
    objc_setAssociatedObject(self, &LFTABLEHEADERViewKey, lf_header, OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"LFTABLEHEADERViewKey"];
}
-(LFTableHeaderView *)lf_header{
    return objc_getAssociatedObject(self, &LFTABLEHEADERViewKey);
}

-(void)setIsShowLFHeader:(NSNumber *)isShowLFHeader{
    [self willChangeValueForKey:@"isShowLFHeader"];
    objc_setAssociatedObject(self, &ShowLFHeader, isShowLFHeader, OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"isShowLFHeader"];
}
-(NSNumber *)isShowLFHeader{
    return objc_getAssociatedObject(self, &ShowLFHeader);
}
...
-(void)addLFHeaderWithClassName:(NSString*)className callback:(void (^)())callback{
    // 1.创建新的header
    if (!self.lf_header) {
        LFTableHeaderView *header = [LFTableHeaderView header];
        header.className = className;
        [self addSubview:header];
        self.lf_header = header;
    }
    
    // 2.设置block回调
    self.lf_header.beginRefreshingCallback = callback;
}

-(void)showLf_header{
    if ([self.isShowLFHeader integerValue] == 0) {
        [self.lf_header beginRefreshing];
        self.isShowLFHeader = @1;
    }
}
-(void)hideLf_header{
//    if ([self.isShowLFHeader integerValue] == 1) {
        [self.lf_header endRefreshing];
        self.isShowLFHeader = @0;
//    }
}
